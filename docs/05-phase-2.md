...
**The scrollbar is drawn, not native, and rides the wall of the opened node.** A native bar can do neither thing this needs: it sits at its scrolling element's own rectangular edge, and this node has an organic, breathing outline that wanders away from any rectangle, so wherever the scrolling element is put the bar is a straight line beside a shape rather than part of it — and it is permanent furniture on a surface that has none. So it is hidden, and a `--mask` thumb is drawn against the panel's right edge instead, fading in while scrolling and out again a second after it stops.

Its inset is measured rather than guessed. On a rendered panel the blob's right edge sits at x=1223 where the panel's own edge is at 1224 — a pixel *inside* it — and above roughly 6% of the panel's height it curves away hard, reaching 1148 by 2%. So the thumb sits six pixels in, on a track spanning 8% to 92% of the height, which keeps it on the wall for the whole of its travel. The scroll container keeps a tab stop so arrow keys still scroll it, exactly as the native bar would have. Contains exactly what `/work/[slug]` contains, from the same content object.
