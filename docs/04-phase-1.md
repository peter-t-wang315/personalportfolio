...
- **Mobile/touch and tablet (per the same `useDeviceTier()` gate):** always-present, low-contrast label below the cluster — no hover state to gate a reveal on, so it cycles through the same phrase pool on a slow timer instead, with the same fade treatment, just without cursor-following. Positioned and spaced (`app/hero-nav.tsx`) using the cluster's real computed on-screen radius so nav and the label clear the cluster rather than crowding it — a flat pixel margin was tried first and broke at a different viewport height, since the cluster's on-screen size is a fixed fraction of viewport height, not a constant.
