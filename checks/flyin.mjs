  ...
  return p.evaluate(() => {
    ...
    const tick = () => {
      ...
      if (q) window.__T.push({
        ...
        fov: q.fov, fly: q.flying, placement: q.placement,
...
function report(label, T, t0, taken) {
  ...
  console.log('  ms    d      z      x     fov  place  home.op  home.z  page   flags');
  ...
  for (const r of rows) {
    ...
    console.log(`  ${String(Math.round(r.t - start)).padStart(5)} ${r.d.toFixed(1).padStart(6)} ${r.z.toFixed(1).padStart(6)} ${r.x.toFixed(1).padStart(6)}  ${r.fov.toFixed(0).padStart(3)}  ${r.placement.toFixed(2)}   ${r.home.op.toFixed(2)}    ${r.home.z.toFixed(1).padStart(5)}  ${r.opacity === null ? '  -  ' : r.opacity.toFixed(2).padStart(5)}  ${flags}`);
  }
  const end = fly.at(-1);
